...
## paths <- list(geo = "datasets/ZWE2020PHIA/datasets/ZIMPHIA 2020 Geospatial Data (DTA).zip",
##               data = "datasets/ZWE2020PHIA/datasets/ZIMPHIA 2020 Household (DTA).zip") %>%
...
phia_path <- "~/Data/household surveys/PHIA/datasets/ZWE2020PHIA/datasets/"

phia_files <- list(geo = "ZIMPHIA 2020 Geospatial Data (DTA).zip",
                   data = "ZIMPHIA 2020 Household (DTA).zip") %>%
  lapply(\(x) file.path(phia_path, x))


geo <- rdhs::read_zipdata(phia_files$geo)

hh <- rdhs::read_zipdata(phia_files$data, "zimphia2020hh.dta")
bio <- rdhs::read_zipdata(phia_files$data, "zimphia2020adultbio.dta")
ind <- rdhs::read_zipdata(phia_files$data, "zimphia2020adultind.dta")
roster <- rdhs::read_zipdata(phia_files$data, "zimphia2020roster.dta")
...
dir.create("check")
ggsave("check/zwe2020phia-survey-region-boundaries.png", p_survey_regions, h = 7, w = 7)
