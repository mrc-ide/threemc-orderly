...
phia_path <- "~/Data/household surveys/PHIA/datasets/ZMB2021PHIA/"

phia_files <- list(geo = "ZAMPHIA 2021 Geospatial Data and Documentation.zip",
                   data = "ZAMPHIA 2021 PU Package.zip") %>%
  lapply(\(x) file.path(phia_path, x))


geo <- rdhs::read_zipdata(phia_files$geo)

hh <- rdhs::read_zipdata(phia_files$data, "zamphia2021hh.dta")
bio <- rdhs::read_zipdata(phia_files$data, "zamphia2021adultbio.dta")
ind <- rdhs::read_zipdata(phia_files$data, "zamphia2021adultind.dta")
roster <- rdhs::read_zipdata(phia_files$data, "zamphia2021roster.dta")
...
phia <- ind %>%
  ...
  select(centroidid, hprovince_za, urban, householdid,
         ...
         intwt0, gender, age,
         mcstatus, mcwhotrad, mctradage_za, mcwhomed, mcmedage_za, mcdetail) %>%
  full_join(
    bio %>%
    filter(bt_status == 1) %>%
...
hh$survey_region_id <- hh$hprovince_za # different in each survey dataset depending on survey stratification
phia$survey_region_id <- phia$hprovince_za # different in each survey dataset depending on survey stratification
...
survey_region_area_id <- c("Central"       = "ZMB_1_10",
                           "Copperbelt"    = "ZMB_1_11",
                           "Eastern"	   = "ZMB_1_12",
                           "Luapula"       = "ZMB_1_13",
                           "Lusaka"        = "ZMB_1_14",
...
dir.create("check")
ggsave("check/zmb2021phia-survey-region-boundaries.png", p_survey_regions, h = 7, w = 7)
...
survey_circumcision <- phia %>%
  ...
  transmute(
    ...
    circ_age = case_when(mcdetail == 1 & mcmedage_za >= 0 ~ mcmedage_za,
                         mctradage_za >= 0 ~ mctradage_za,
                         TRUE ~ NA_integer_),
