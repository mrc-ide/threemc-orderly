...
## paths <- list(geo = "datasets/SWZ2021PHIA/datasets/SHIMS3 2021 Geospatial Data (DTA).zip",
##               data = "datasets/SWZ2021PHIA/datasets/SHIMS3 2021 Household (DTA).zip") %>%
...
phia_path <- "~/Data/household surveys/PHIA/datasets/SWZ2021PHIA/datasets"

phia_files <- list(geo = "SHIMS3 2021 Geospatial Data (DTA).zip",
                   data = "SHIMS3 2021 Household Interview and Biomarker Datasets (DTA).zip") %>%
  lapply(\(x) file.path(phia_path, x))


geo <- rdhs::read_zipdata(phia_files$geo)

hh <- rdhs::read_zipdata(phia_files$data, "shims32021hh.dta")
bio <- rdhs::read_zipdata(phia_files$data, "shims32021adultbio.dta")
ind <- rdhs::read_zipdata(phia_files$data, "shims32021adultind.dta")
roster <- rdhs::read_zipdata(phia_files$data, "shims32021roster.dta")
...
phia <- ind %>%
  ...
  select(centroidid, region, urban, householdid,
         ...
         intwt0, gender, age,
         mcstatus_sz, mcage, mcwhomed, mcwhotrad, mcdetail) %>%
  full_join(
    bio %>%
      filter(bt_status == 1) %>%
...
dir.create("check")
ggsave("check/swz2021phia-survey-region-boundaries.png", p_survey_regions, h = 7, w = 7)
...
survey_clusters <- survey_clusters %>%
  left_join(
    survey_region_areas, by = "survey_region_id",
...
survey_circumcision <- phia %>%
  ...
  transmute(
    survey_id,
    individual_id = personid,
    circumcised = recode(as.integer(mcstatus_sz),
