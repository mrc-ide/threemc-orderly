...
#### Load ZAF fit & use fixed age hyperpars ####

# load ZAF fit
zaf_fit <- readRDS("depends/TMBObjects_DistrictAgeTime_ByType.rds")

# set time variance hyperparameters to "test" values
zaf_hyperpars <- zaf_fit$par
# remove agetime components
zaf_hyperpars <- zaf_hyperpars[!grepl("agetime", names(zaf_hyperpars))]
# only keep age related hyperparameters
zaf_hyperpars_age <- zaf_hyperpars[grepl("age", names(zaf_hyperpars))]

param_order <- names(parameters)
parameters <- parameters[!names(parameters) %in% names(zaf_hyperpars_age)]
parameters <- c(parameters, as.list(zaf_hyperpars_age))
...
maps <- lapply(rep(NA, length(zaf_hyperpars_age)), factor)
names(maps) <- names(zaf_hyperpars_age)
